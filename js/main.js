...
/* harmony import */ var _components_yandex_map_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./components/yandex-map.js */ "./src/js/components/yandex-map.js");
...
/* harmony import */ var _vendor_accordion_min_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./vendor/accordion.min.js */ "./src/js/vendor/accordion.min.js");
/* harmony import */ var _vendor_accordion_min_js__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_vendor_accordion_min_js__WEBPACK_IMPORTED_MODULE_1__);
...
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
        ...
        "use strict";
        __webpack_require__.r(__webpack_exports__);
/* harmony import */ var _vendor_accordion_min_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../vendor/accordion.min.js */ "./src/js/vendor/accordion.min.js");
/* harmony import */ var _vendor_accordion_min_js__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_vendor_accordion_min_js__WEBPACK_IMPORTED_MODULE_0__);

        new (_vendor_accordion_min_js__WEBPACK_IMPORTED_MODULE_0___default())('.accordion-container');
...
/***/ (() => {
        ...
        let headerFixedHidden = true;
        const header = document.querySelector('.header');
        const nav = document.querySelector('.header .nav');
        const headerFixed = document.querySelector('.header-fixed');
        const siteContainer = document.querySelector('.site-container');
        const heightHeader = header.offsetHeight;
        function activeHeaderFixed() {
          const burgerMenuActive = document.querySelector('.menu--active');
          if (burgerMenuActive) return;
          const afterHeightHeader = window.scrollY > heightHeader * 2 + 30;
          if (afterHeightHeader && headerFixedHidden) {
            headerFixedHidden = false;
            siteContainer.style.paddingTop = heightHeader + 'px';
            header.classList.add('is-hidden');
            headerFixed.classList.add('active');
            headerFixed.classList.remove('fadeOutUp');
            headerFixed.classList.add('fadeInUp');
          } else if (!afterHeightHeader && !headerFixedHidden) {
            headerFixedHidden = true;
            siteContainer.style.paddingTop = '';
            header.classList.remove('is-hidden');
            headerFixed.classList.remove('fadeInUp');
            headerFixed.classList.add('fadeOutUp');
            setTimeout(() => {
              headerFixed.classList.remove('active');
            }, 300);
          }
        }
        window.addEventListener('scroll', activeHeaderFixed);
...
/* harmony export */   showResults: () => (/* binding */ showResults)
        ...
        const cleaner = document.querySelector('.search__cleaner');
        ...
        input.addEventListener('input', e => {
          ...
          results.classList.toggle('active', valueLength);
          cleaner.classList.toggle('active', valueLength);
          results.innerHTML = '<span style="padding:20px 10px">Поиск...</span>';
        ...
        input.addEventListener('blur', e => {
          const value = e.target.value.trim();
          input.classList.toggle('value-full', value.length);
        });
        cleaner.addEventListener('click', () => {
          input.value = '';
          input.classList.remove('value-full');
          results.classList.remove('active');
          cleaner.classList.remove('active');
        });
        results.addEventListener('click', e => {
          const target = e.target;
          const clickInItem = target.closest('.search__results-item');
          if (clickInItem) results.classList.remove('active');
        });
        document.addEventListener('click', e => {
          const clickInSearch = search.contains(e.target);
          if (!clickInSearch) results.classList.remove('active');
        });
        function showResults(resultsData) {
          ...
          if (resultsData.length) {
            ...
            resultsData.forEach(obj => {
              const item = document.createElement('li');
              item.classList.add('search__results-item');
              item.tabIndex = '0';
              item.dataset.value = obj.value;
              item.textContent = obj.displayName;
              list.append(item);
            });
            results.append(list);
          } else {
            results.innerHTML = '<span style="padding:20px 10px">Нет результата по данному запросу</span>';
          }
        }
...
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
        ...
        "use strict";
        __webpack_require__.r(__webpack_exports__);
/* harmony import */ var _data_data_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../data/data.js */ "./src/js/data/data.js");

        const defaultSettings = {
          zoom: 10,
          type: 'yandex#map',
          center: [55.748203, 37.615272]
        };
        ...
        function init() {
          ...
          objectManager.objects.options.set({
            ...
            balloonMinWidth: '293',
            balloonMaxWidth: '293',
            balloonMinHeight: '290',
            balloonMaxHeight: '290',
            balloonContentLayout: createBalloon()
          });
          objectManager.objects.events.add('balloonopen', function (event) {
            resetStylesPrevPoint();
            onPointClick(event);
          });
          map.events.add('boundschange', function () {
            const points = getVisiblePoints();
            const data = getDataVisiblePoints(points);
            renderBoxAddressList(data);
          });
          objectManager.clusters.options.set({
            clusterIconLayout: createCluster(),
            clusterIconShape: createIconShape()
          });
          function addSwitchedPointOnBox() {
            const addressList = document.querySelector('.address__list');
            const boxAddressItems = document.querySelectorAll('.box-address');
            boxAddressItems.forEach(boxAddress => {
              const pointActive = boxAddress.dataset.idPoint === addressList.dataset.idClick;
              if (pointActive) {
                boxAddress.classList.add('active');
              }
              boxAddress.addEventListener('click', e => handlerBoxAddress(e));
              boxAddress.addEventListener('keypress', e => {
            ...
            function handlerBoxAddress(event) {
              addressList.dataset.idClick = event.currentTarget.dataset.idPoint;
              boxAddressItems.forEach(it => it.classList.remove('active'));
              event.currentTarget.classList.add('active');
              curIdPoint = event.currentTarget.dataset.idPoint;
              resetStylesPrevPoint();
              setOptionsPoint(curIdPoint);
              // renderAllPoints()
              goToPoint(curIdPoint);
              prevIdPoint = curIdPoint;
            }
          }
          function addToPointOnResults() {
            const inputSearch = document.querySelector('.search__input');
            const results = document.querySelector('.search__results');
            results.addEventListener('click', e => {
              const target = e.target;
              const resultsItem = target.closest('.search__results-item');
              if (!resultsItem) return;
              results.classList.remove('active');
              const pointName = resultsItem.dataset.value;
              inputSearch.value = pointName;
              ymaps.geocode(pointName, {
          ...
          function goToPoint(idPoint) {
            console.log(idPoint);
            const coordinates = objectManager.objects.getById(idPoint).geometry.coordinates;
            map.setCenter(coordinates, zoomToPoint, {
              checkZoomRange: true
            });
          }
          function getVisiblePoints() {
            return ymaps.geoQuery(objectManager.objects).searchInside(map).map(function (object) {
              // console.log(object)
              return object;
            })._objects;
          }
          function getDataVisiblePoints(points) {
            const arrIdPoints = points.map(obj => obj.properties.get('id'));
            const visiblePointsData = _data_data_js__WEBPACK_IMPORTED_MODULE_0__.dataPoints.filter(point => arrIdPoints.includes(point.properties.id));
            return visiblePointsData;
          }
          function renderAllPoints() {
            _data_data_js__WEBPACK_IMPORTED_MODULE_0__.dataPoints.forEach(obj => {
              objectManager.add({
                ...obj
              });
          ...
          function createBoxAddressList(dataPoints) {
            const addressList = document.querySelector('.address__list');
            addressList.innerHTML = '';
            dataPoints.forEach(point => {
              const addressItem = document.createElement('li');
              addressItem.classList.add('address__item');
              const box = document.createElement('div');
              box.classList.add('box-address');
              box.tabIndex = '0';
              box.dataset.idPoint = point.id;
              const title = document.createElement('h3');
              title.classList.add('box-address__title');
              title.textContent = point.properties.header;
              const address = document.createElement('a');
              address.classList.add('box-address__address');
              address.textContent = point.properties.header;
              address.href = point.properties.addressOnMap;
              const boxList = document.createElement('ul');
              boxList.classList.add('list-reset', 'box-address__list');
              const boxItemClock = document.createElement('li');
              boxItemClock.classList.add('box-address__item', 'box-address__item_clock');
              point.properties.balloonTimeWork.forEach(time => {
                const span = document.createElement('span');
                span.classList.add('box-address__text');
                span.textContent = time;
                boxItemClock.append(span);
              });
              const boxItemPhone = document.createElement('li');
              boxItemPhone.classList.add('box-address__item', 'box-address__item_phone');
              point.properties.balloonContacts.forEach(contact => {
                const phone = document.createElement('a');
                phone.classList.add('box-address__text');
                phone.href = 'tel:79992532312';
                phone.textContent = point.properties.balloonContacts;
                boxItemPhone.append(phone);
              });
              addressList.append(addressItem);
              addressItem.append(box);
              box.append(title);
              box.append(address);
              box.append(boxList);
              boxList.append(boxItemClock);
              boxList.append(boxItemPhone);
            });
          }
          function renderBoxAddressList(dataPoints) {
            createBoxAddressList(dataPoints);
            addSwitchedPointOnBox();
            addToPointOnResults();
          }
          function resetStylesPrevPoint() {
            if (!prevIdPoint.length) return;
            objectManager.objects.setObjectOptions(prevIdPoint, {
              ...defaultPointSize
            });
          }
          function setOptionsPoint(pointId) {
          ...
          function onPointClick(e) {
            ...
            prevIdPoint = pointId;
            setOptionsPoint(pointId);
            const target = e.get('target');
            const zoom = target._oldMapZoom;
            if (zoom < zoomToPoint) {
              goToPoint(pointId);
            }
          }
          ...
          function createCluster() {
            return ymaps.templateLayoutFactory.createClass(`<div class="cluster-map">
          <svg>
            <use xlink:href="#point-map" style="fill: #160b75">
            </use>
          </svg>
          <span class="cluster-map__number">
  ...
  <div class="box-address">
    <h3 class="box-address__title">
      {{ properties.header|default:"Название оптики" }}
    </h3>
    <a class="box-address__address" href="{{properties.addressOnMap}}">
      {{ properties.address | default:"Адрес оптики" }}
    </a>

    <ul class="list-reset box-address__list">
      <li class="box-address__item box-address__item_clock">
        <div class="box-address__item-title">
          Время работы
        </div>
        <div>
        {% for time in properties.balloonTimeWork %}
          <span class="box-address__text"> {{ time | default: '-' }}</span>
        {% endfor %}
        </div>
      </li>
      <li class="box-address__item box-address__item_phone">
        <div class="box-address__item-title">
          Контакты
        </div>
        {% for contact in properties.balloonContacts %}
          <a href="tel:{{ contact.replace(/\D/g, '') | default: "" }}" class="box-address__text">
            {{ contact }}
          </a>
        {% endfor %}
      </li>
...
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
        ...
        __webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   dataPoints: () => (/* binding */ dataPoints)
          /* harmony export */
});
        const dataPoints = [{
          type: 'Feature',
          ...
          geometry: {
            type: 'Point',
            coordinates: [55.682838, 37.537047]
          },
          properties: {
            id: 'id1',
            header: 'Aycraft Optics',
            hintContent: '109052, с. Moscow, Ryazansky Ave. 2 k. 2 / Aycraft Optics',
            address: '109052, с. Moscow, Ryazansky Ave. 2 k. 2',
            addressOnMap: 'https://yandex.ru/maps/213/moscow/house/ryazanskiy_prospekt_2k2/Z04YcQRgTUYAQFtvfXtzeHRkZw==/?indoorLevel=1&ll=37.730437%2C55.729874&z=16',
            balloonTimeWork: ["Mo-Fr 9:00-20:00", "Mo-Fr 9:00-20:00"],
            balloonContacts: ['+7 (999) 253-23-12']
          }
        }, {
          type: 'Feature',
          id: 'id2',
          geometry: {
            type: 'Point',
            coordinates: [55.684861, 37.540257]
          },
          properties: {
            id: 'id2',
            header: 'Optical City',
            hintContent: '109004, с. Moscow, Zemlyanoy Val street, v. 54 p. 2 / Optical City',
            address: '109004, с. Moscow, Zemlyanoy Val street, v. 54 p. 2',
            addressOnMap: 'https://yandex.ru/maps/213/moscow/house/ryazanskiy_prospekt_2k2/Z04YcQRgTUYAQFtvfXtzeHRkZw==/?indoorLevel=1&ll=37.730437%2C55.729874&z=16',
            balloonTimeWork: ["Mo-Fr 9:00-20:00", "Mo-Fr 9:00-20:00"],
            balloonContacts: ['+7 (999) 253-23-12']
          }
          ...
          geometry: {
            type: 'Point',
            coordinates: [55.830589, 37.728657]
          },
          properties: {
            id: 'id3',
            header: 'Linzmaster',
            hintContent: 'с. Moscow, Varshavskoe shosse, vld 140 / Linzmaster',
            address: 'с. Moscow, Varshavskoe shosse, vld 140',
            addressOnMap: 'https://yandex.ru/maps/213/moscow/house/ryazanskiy_prospekt_2k2/Z04YcQRgTUYAQFtvfXtzeHRkZw==/?indoorLevel=1&ll=37.730437%2C55.729874&z=16',
            balloonTimeWork: ["Mo-Fr 9:00-20:00", "Mo-Fr 9:00-20:00"],
            balloonContacts: ['+7 (999) 253-23-12']
          }
          ...
          geometry: {
            type: 'Point',
            coordinates: [55.748203, 37.615272]
          },
          properties: {
            id: 'id4',
            header: 'Ocharik',
            hintContent: '123060, Moscow, ul. Marshal Biryuzova, 32 / Ocharik',
            address: '123060, Moscow, ul. Marshal Biryuzov, 32',
            addressOnMap: 'https://yandex.ru/maps/213/moscow/house/ryazanskiy_prospekt_2k2/Z04YcQRgTUYAQFtvfXtzeHRkZw==/?indoorLevel=1&ll=37.730437%2C55.729874&z=16',
            balloonTimeWork: ["Mo-Fr 9:00-20:00", "Mo-Fr 9:00-20:00"],
            balloonContacts: ['+7 (999) 253-23-12']
          }
          ...
          geometry: {
            type: 'Point',
            coordinates: [55.803535, 37.552464]
          },
          properties: {
            id: 'id5',
            header: 'Ocharik',
            hintContent: '12301, Savelovsky district, Northern Administrative District, Moscow / Ochkarik-plus',
            address: '12301, Savelovsky district, Northern Administrative District, Moscow',
            addressOnMap: 'https://yandex.ru/maps/213/moscow/?from=api-maps&ll=37.621815%2C55.753033&mode=whatshere&origin=jsapi_2_1_79&whatshere%5Bpoint%5D=37.552464%2C55.803535&whatshere%5Bzoom%5D=11&z=11',
            balloonTimeWork: ["Mo-Fr 9:00-20:00", "Mo-Fr 9:00-20:00"],
            balloonContacts: ['+7 (999) 153-43-12']
          }
        }, {
          type: 'Feature',
          id: 'id6',
          geometry: {
            type: 'Point',
            coordinates: [55.729985, 37.679494]
          },
          properties: {
            id: 'id6',
            header: 'Ocharik+',
            hintContent: '222060, Tagansky District, Central Administrative District, Moscow / Ochkarik+',
            address: '222060, Tagansky District, Central Administrative District, Moscow',
            addressOnMap: 'https://yandex.ru/maps/213/moscow/?from=api-maps&ll=37.621815%2C55.753033&mode=whatshere&origin=jsapi_2_1_79&whatshere%5Bpoint%5D=37.679494%2C55.729985&whatshere%5Bzoom%5D=11&z=11',
            balloonTimeWork: ["Mo-Fr 9:00-20:00", "Mo-Fr 9:00-20:00"],
            balloonContacts: ['+7 (999) 555-23-12']
          }
        }];
...
/* harmony import */ var _functions_enable_scroll__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../functions/enable-scroll */ "./src/js/functions/enable-scroll.js");
        ...
        (function () {
          const burgersList = document?.querySelectorAll('[data-burger]');
          ...
          burgersList?.forEach(burger => {
            burger.addEventListener('click', () => {
              ...
              if (menu?.classList.contains('menu--active')) {
                addAnimationHidden(burger);
                burger?.setAttribute('aria-expanded', 'false');
                burger?.setAttribute('aria-label', 'Открыть меню');
                (0, _functions_enable_scroll__WEBPACK_IMPORTED_MODULE_1__.enableScroll)();
                hiddenMenu();
              } else {
                addAnimationVisible(burger);
          ...
          function addAnimationHidden(elem) {
          ...
          function addAnimationVisible(elem) {
            elem?.classList.add('burger--active');
...
/***/ "./src/js/vendor/accordion.min.js":
/*!****************************************!*\
  !*** ./src/js/vendor/accordion.min.js ***!
  \****************************************/
/***/ ((module) => {
